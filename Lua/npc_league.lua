...
hook.Add ("Update", "League_Update", Update)
...
function Update2()
    ...
    --            NPC:SayPrivateMessage(client, translator.AdvTranslate(client, "Greetings", { name = client.Name }))
...
hook.Add ("Update2", "League_Update2", Update2)
...
function PrivateMessage (client, message)
    ...
    if message == "/tournament" and (not ClientExtraData[client.ID].TournamentRequest or ClientExtraData[client.ID].TournamentRequest == "no") then
        NPC:SayPrivateMessage (client, translator.AdvTranslate (client, "Tournament1", { name = Tournament.Name }))
    ...
    if message == "yes" or message == "yep" and (not ClientExtraData[client.ID].TournamentRequest or ClientExtraData[client.ID].TournamentRequest == "waiting") then
        NPC:SayPrivateMessage (client, translator.AdvTranslate (client, "Tournament2", { name = Tournament.Name }))
    ...
    if message == "no" or message == "nope" and (not ClientExtraData[client.ID].TournamentRequest or ClientExtraData[client.ID].TournamentRequest == "waiting") then
        NPC:SayPrivateMessage (client, translator.AdvTranslate (client, "Tournament3", { name = Tournament.Name }))
...
hook.Add ("PrivateMessage", "League_PrivateMessage", PrivateMessage)